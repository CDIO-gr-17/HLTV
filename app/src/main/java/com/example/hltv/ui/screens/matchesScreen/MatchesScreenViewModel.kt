...
class MatchesScreenViewModel : ViewModel() {
    ...
    val matchesFromDay = mutableStateListOf<Event>()
    ...
    suspend fun loadUpcomingMatches(day: Long = 0) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            do {
                val upcomingMatches: APIResponse.EventsWrapper
                if (day == 0.toLong()) {
                    upcomingMatches =
                        getMatchesFromDay(convertTimestampToDateURL((nextDayInSeconds).toInt()))
                } else {
                    MAXNUMBEROFDAYSTOLOAD = 1
                    upcomingMatches =
                        getMatchesFromDay(convertTimestampToDateURL((day).toInt()))

                }

                //the following is to shit yourself in the pants.
                dayWasEmpty = true
                ...
                if (dayWasEmpty) {
                    Log.i(
                        "loadUpcomingMatches()",
                        "nextdayinsceonds : $nextDayInSeconds. Current day was empty"
                    )
                } else {
                    Log.i(
                        "loadUpcomingMatches()",
                        "nextdayinsceonds : $nextDayInSeconds. Current day was not empty"
                    )
                }
        ...
        viewModelScope.launch {
            CoroutineScope(Dispatchers.IO).launch {
                if (value != 0.toLong()) {
                    ...
                    loadUpcomingMatches(value)
                }
            }
        }
    }

    fun loadMatchesFromDay(timeStamp: Long) {
        CoroutineScope(Dispatchers.IO).launch {
            val matches = getMatchesFromDay(timeStamp.toString())
            if (matches.events.isNotEmpty()) {
                matches.events.forEachIndexed() { index, event ->
                    matchesFromDay.add(event)
                    homeTeamIcons[index] = (getTeamImage(event.homeTeam.id))
                    awayTeamIcons[index] = (getTeamImage(event.awayTeam.id))
                }
            } else {
                Log.w(this.toString(), "There were no matches on this day?")
            }
